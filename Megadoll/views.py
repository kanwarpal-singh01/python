...
import requests, json, stripe, ast, random, taxjar
...
def get_shipping_price_range(shipping_prices, sub_total):
    max_shipping_price = shipping_prices.order_by('-order_value_upto')[0]
    for shipping_price in shipping_prices:
            if shipping_price.order_value_from >= sub_total and shipping_price.order_value_upto <= sub_total:
                return shipping_price
            elif shipping_price > max_shipping_price:
                return 0
    return shipping_prices.last()
...
def billing(request):
    try:
        ...
        data = {
            ...
            'grand_total': round(sub_total + shipping_price_range.shipping_cost, 2)
        }
...
def tax_amount(user_id, shipping_address_id, shipping_cost):
    user_address = UserAddress.objects.get(id=int(shipping_address_id), user_id=user_id)
    user_cart = UserCart.objects.get(user_id=user_id)
    shop_product_list = string_to_list(user_cart.shop_product_list)
    product_dict = dict(Counter(shop_product_list))
...
def guest_user_tax_amount(guest_user_data, shipping_cost):
    data = {
        'address': guest_user_data.shipping_address, 
        'shipping_cost': shipping_cost,
        'product_detail': guest_user_data.order_detail,
    }
    tax = float(guest_user_taxjar_tax_amount(data))
    return round(tax, 2)
...
def save_order_data_to_session(request):
    ...
    tax = tax_amount(request.user.id, shipping_address_id, shipping_cost)

    total_amount = round(sub_total + shipping_cost + tax, 2)
...
def guest_user_save_order_data_to_session(request):
    ...
    if guest_user_data:
        ...
        tax = float(guest_user_tax_amount(guest_user_data, shipping_cost))
...
def guest_user_billing(request):
    ...
    guest_user.total_amount = sub_total
    ...
    data = {
        ...
        'shipping_prices': shipping_prices,
        'shipping_price_range': shipping_price_range,
        'grand_total': round(float(sub_total) + float(guest_user.shipping_charge), 2)
    }
...
def render_sloper_landing_page(request):
    ...
    individual_sloper_plans = SloperPlan.objects.filter(category_id = individual_sloper.id)
...
def summary_sloper_individual_personal_plan(request):
    total_subscription_amount = request.POST["total_amount"]
...
def get_stripe_data_for_sloper_subscription(stripe_payment_intent_id):
    ...
    return {'reciept_url': reciept_url, 'stripe_data': stripe_data}
...
def save_sloper_hospital_data(request):
    ...
    patient_count = request.POST["patient_count"]
    discount = request.POST["discount"]
    original_price = float(total_price) / (1-(float(discount)/100))
    discount_price = original_price-float(total_price)
...
def select_hospital_for_sloper_subscription(request):
    hospitals = SloperHospital.objects.filter(is_active = True)
    return render(request, "pages/sloper-subscription/select-hospital.html", {"hospitals": hospitals})    


def get_search_hospital_data(request):
    words = request.POST["words"]
    hospitals = SloperHospital.objects.filter(name__icontains = words, is_active = True).values()
...
def sloper_hospital_order_confirmation(request):
    ...
    total_amount_on_stripe = plan_data["total_price"]
    discount = plan_data["discount"]
...
def pay_school_subscription(request):
    no_of_month = request.POST['no_of_month']
    no_of_student = request.POST['no_of_student']
    no_of_teacher = request.POST['no_of_teacher']
    student_price = request.POST['student_price']
    teacher_price = request.POST['teacher_price']
    total_price = request.POST['total_price']
...
@login_required
def activate_sloper_subscription(request):
    time = timezone.now()
    user_id = request.user.id     
...
def cancel_sloper_subscription(request):
    time = timezone.now()
    subscription_id = request.POST['subscription_id']
    subscription_data = UserSloperSubscription.objects.get(id=subscription_id)
    subscription_data.is_canceled = False
    subscription_data.is_canceled = True
    subscription_data.canceled_at = time
    subscription_data.save()
    sloper_stripe_data = SloperStripeData.objects.get(id = subscription_data.sloper_stripe_data_id)
    return JsonResponse({'success': False, 'message': 'plan canceled'})
