                    ...
                    if GoalLeaveRequest.objects.filter(goal_id=data['goal_id'], user_id=data['user_id']).exists():
                        if data['approve'] == '1':
                            ...
                            for i in members:
                                ...
                                create_poll = GoalPoll.objects.create(goal_id=data['goal_id'], leave_user_id=data['user_id'], goal_member_id=i.members_id, due_date=newdate, remove_self=1)
                                create_poll.save()
                            return Response({
                                'status': True, 
                    ...
                    if GoalGroupAdmin.objects.filter(group_goal_id=data['goal_id'], user_id=user_data.id, approve=1).exists():
                        if data['remove'] == '1':
                            ...
                            for i in members:
                                ...
                                create_poll = GoalPoll.objects.create(goal_id=data['goal_id'], leave_user_id=data['user_id'], goal_member_id=i.members_id, due_date=newdate, remove_admin=1)
                                create_poll.save()
                            return Response({
                                'status': True, 
    ...
    def get(self, request):
        try:
            ...
            if user_data.user_type == 'VENDOR' and user_data.is_active == True:
                ...
                pagination_data = paginat.paginate_queryset(vendor_invoice, request)
                invoice_serializer = VendorTransactionSerializer(pagination_data, many=True).data
                pagination_serializer = paginat.get_paginated_response(invoice_serializer).data
                return Response({
                    'status': True, 
                    'payload': pagination_serializer,
...
def goalSubscriptionCompletedwebhook(request):
    endpoint_secret = 'whsec_748cc594a601cb3796b39424a64e011442644de765e378cb246ba4fab3315ba7'
    event = None
    ...
    if event['type'] == 'invoice.paid':
        ...
        if int(goal_collect_amount['goal_amount']) >= int(user_goal.goal_amount):
            ...
            if user_goal.goal_as == 'PRODUCT':
                ...
                goal_order.save()
            #     VendorInvoice.objects.create(amount=user_goal.goal_amount, goal_id=user_goal.id, order_id=goal_order.order_id)
            #     vendor_product = user_goal.product
            #     sendProductGoalCompleteMail(user_goal, vendor_product)
        else:
            pass
    else:
